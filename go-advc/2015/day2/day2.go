...
// Fortunately, every present is a box (a perfect right rectangular prism), which makes calculating the required wrapping paper for each gift a
// little easier: find the surface area of the box, which is 2*l*w + 2*w*h + 2*h*l. The elves also need a little extra paper for each present:
...
func part1() int {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		if err != nil {
			fmt.Println("Error converting dimensions to integers")
			break
		}
