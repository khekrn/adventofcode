...
public class Day1 {
    ...
    // the directions he got are a little confusing. He starts on the ground floor
    // (floor 0) and then
    ...
    private int part1() {
        var floor = 0;
        try {
            var data = Files.readAllBytes(Path.of("src/main/resources/2015/Day1.txt"));
            for (byte b : data) {
                if (b == '(') {
                    floor++;
                } else if (b == ')') {
