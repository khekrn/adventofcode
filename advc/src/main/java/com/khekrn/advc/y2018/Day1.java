...
public class Day1 {
     ...
     * Below the message, the device shows a sequence of changes in frequency (your
     * puzzle input). A value like +6 means the current frequency increases by 6; a
    ...
    private static int part1() throws Exception {
        var result = 0;
        try (Stream<String> lines = Files.lines(Paths.get("src/main/resources/Day1.txt"))) {
            result = lines.mapToInt(x -> Integer.parseInt(x)).sum();
        }
        return result;
    }
